    ...
    addScrollPagingEvent(){
        ...
        window.onscroll = () => {
            ...
            if(scrollStatus > -50 && scrollStatus <50 ){
               const noewPage =  CollectionsService.getInstance().collectionsEntity.page;
               CollectionsService.getInstance().collectionsEntity.page = Number(noewPage) + 1;
    ...
    loadCollections(){
        if(this.collectionsEntity.page ==1 || this.collectionsEntity.page < Number(this.collectionsEntity.maxPage) + 1 ) {
            ...
            if(responseData.length > 0){
                ...
                alert("해당 카테코리에 등록된 상품 정보가 없습니다.");
